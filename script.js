...
function mettreAJourCalques() {
    // On nettoie tout avant de reconstruire
    ...
    // ÉTAPE A : On place les éléments DERRIÈRE le mannequin
ORDRE_DESSOUS.forEach(cat => {
    const fichier = (apercu && cat === categorieActuelle) ? apercu : selections[cat];
    if (fichier) {
        const img = document.createElement('img');
        img.src = fichier;
        img.className = 'calque-derriere'; // ← était 'calque-vetement'
...
const pupilles = [
  { el: null, ox: 0.25, oy: 0.5 }, // ← pupille gauche
  { el: null, ox: 0.80, oy: 0.5 }, // ← pupille droite
];

pupilles.forEach((p, i) => {
  const div = document.createElement('div');
  div.style.cssText = 'position:absolute;width:150px;height:12px;border-radius:50%;background:transparent;pointer-events:none;z-index:3;';
  document.getElementById('vue-accueil').appendChild(div);
  p.el = div;
});

document.addEventListener('mousemove', (e) => {
  const yeux = document.getElementById('yeux-accueil');
  if (!yeux) return;
  const rect = yeux.getBoundingClientRect();
  pupilles.forEach(p => {
    const cx = rect.left + rect.width * p.ox;
    const cy = rect.top + rect.height * p.oy;
    const dx = e.clientX - cx;
    const dy = e.clientY - cy;
    const angle = Math.atan2(dy, dx);
    const dist = 5;
    p.el.style.left = (rect.left + rect.width * p.ox + Math.cos(angle) * dist - 6) + 'px';
    p.el.style.top = (rect.top + rect.height * p.oy + Math.sin(angle) * dist - 6) + 'px';
  });
});